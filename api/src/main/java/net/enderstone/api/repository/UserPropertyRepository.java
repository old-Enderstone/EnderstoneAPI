...
public class UserPropertyRepository {
    ...
    public void setValue(UUID user, UserProperty property, Object value) {
        final Message message = IOUtils.getJson(String.format("%s%s/%s/%s/%s",
                                                              api.getBaseUrl(),
                                                              "/set/player/property",
                                                              user.toString(),
                                                              property.toString(),
    ...
    public int addInt(UUID user, UserProperty property, int value) {
        final TypedMessage<Integer> message = IOUtils.getTypedMessage(String.format("%s%s/%s/%s/%d",
                                                                                    api.getBaseUrl(),
                                                                                    "/addInt/player/property",
                                                                                    user.toString(),
                                                                                    property.toString(),
                                                                                    value), Integer.class);
        if(message == null || message.id() != 200) throw new RuntimeException("Failed to set user property.");
    ...
    public long addLong(UUID user, UserProperty property, long value) {
        final TypedMessage<Long> message = IOUtils.getTypedMessage(String.format("%s%s/%s/%s/%d",
                                                                                 api.getBaseUrl(),
                                                                                 "/addLong/player/property",
                                                                                 user.toString(),
                                                                                 property.toString(),
                                                                                 value), Long.class);
        if(message == null || message.id() != 200) throw new RuntimeException("Failed to set user property.");
    ...
    public double addDouble(UUID user, UserProperty property, double value) {
        final TypedMessage<Double> message = IOUtils.getTypedMessage(String.format("%s%s/%s/%s/%f",
                                                                                   api.getBaseUrl(),
                                                                                   "/addDouble/player/property",
                                                                                   user.toString(),
                                                                                   property.toString(),
                                                                                   value), Double.class);
        if(message == null || message.id() != 200) throw new RuntimeException("Failed to set user property.");
    ...
    public float addFloat(UUID user, UserProperty property, float value) {
        final TypedMessage<Float> message = IOUtils.getTypedMessage(String.format("%s%s/%s/%s/%f",
                                                                                  api.getBaseUrl(),
                                                                                  "/addFloat/player/property",
                                                                                  user.toString(),
                                                                                  property.toString(),
                                                                                  value), Float.class);
        if(message == null || message.id() != 200) throw new RuntimeException("Failed to set user property.");
